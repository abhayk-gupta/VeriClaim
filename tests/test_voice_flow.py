...
def test_greeting_menu_has_gather_and_options():
...
def test_ask_order_id_gathers_input():
    xml = twiml.ask_order_id()
    assert "<Gather" in xml
    assert "/webhooks/voice/collect-order-id" in xml


def test_ask_claim_type_lists_options():
...
def test_clarification_question_records_to_callback(claim_id):
